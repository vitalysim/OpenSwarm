...
export let list: () => Promise<Record<ProviderID, Info>> | Record<ProviderID, Info> = () =>
  run(
    Effect.gen(function* () {
...
export const getProvider = (providerID: ProviderID) =>
  run(
    Effect.gen(function* () {
      const provider = yield* Service
      return yield* provider.getProvider(providerID)
    }),
  )
...
export const getLanguage = (model: Model) =>
  run(
    Effect.gen(function* () {
      const provider = yield* Service
