...
export const create = (input?: Parameters<Interface["create"]>[0]) =>
  run(
    Effect.gen(function* () {
...
export const messages = (input: Parameters<Interface["messages"]>[0]) =>
  run(
    Effect.gen(function* () {
      const session = yield* Service
      return yield* session.messages(input)
    }),
  )

export const updateMessage = <T extends MessageV2.Info>(msg: T) =>
  run(
    Effect.gen(function* () {
      const session = yield* Service
...
export const updatePart = <T extends MessageV2.Part>(part: T) =>
  run(
    Effect.gen(function* () {
      const session = yield* Service
...
export const remove = (sessionID: SessionID) =>
  run(
    Effect.gen(function* () {
      const session = yield* Service
      return yield* session.remove(sessionID)
    }),
  )
