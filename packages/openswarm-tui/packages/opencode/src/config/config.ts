...
function globalConfigFile() {
  const candidates = [
    ...AgencyBrand.configFilesPreferred,
    ...AgencyBrand.legacyConfigFiles.toReversed(),
    "config.json",
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      for (const file of AgencyBrand.legacyConfigFiles) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files(AgencyBrand.config, ctx.directory, ctx.worktree).pipe(
            Effect.orDie,
          )) {
        ...
        // level. Keep an explicit legacy `.opencode` override in place, but still let an explicit
        // branded `.agentswarm` override its legacy sibling.
        ...
        for (const dir of ordered) {
          if (
            ...
            for (const file of [...AgencyBrand.legacyConfigFiles, ...AgencyBrand.configFiles]) {
...
const run = <A, E>(effect: Effect.Effect<A, E, Service>) => Effect.runPromise(effect.pipe(Effect.provide(defaultLayer)))
...
export const updateGlobal = (input: Info) =>
  run(
    Effect.gen(function* () {
      const config = yield* Service
      return yield* config.updateGlobal(input)
    }),
  )
